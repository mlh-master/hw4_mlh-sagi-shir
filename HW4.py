...
from tensorflow.keras.layers import Flatten, InputLayer, LeakyReLU
...
if not("results" in os.listdir()):
    os.mkdir("results")
save_dir = "results/"
model_name = "final_weights_RelU_64bs.h5"
model_path = os.path.join(save_dir, model_name)
model_relu.save(model_path)

cur_model = load_model("results/final_weights_RelU_64bs.h5")
loss_and_metrics = cur_model.evaluate(X_test,Y_test, verbose = 2)
print("RelU activation, 25 epochs, batch size 64 model - test set performance:\n Accuracy: %f\n Loss: %f" % (loss_and_metrics[1], loss_and_metrics[0]))
#----------------------------------------------------------------------------------------
...
# new_a_model.add(LeakyReLU(alpha=0.1))
# new_a_model.add(Activation('tanh'))
new_a_model.add(Activation('sigmoid'))
...
if not("results" in os.listdir()):
    os.mkdir("results")
save_dir = "results/"
model_name = "init_weights_Sigmoid.h5"
model_path = os.path.join(save_dir, model_name)
...
if not("results" in os.listdir()):
    os.mkdir("results")
save_dir = "results/"
model_name = "final_weights_Sigmoid_25ep.h5"
model_path = os.path.join(save_dir, model_name)
new_a_model.save(model_path)

cur_model = load_model("results/final_weights_Sigmoid_25ep.h5")
loss_and_metrics = cur_model.evaluate(X_test,Y_test, verbose = 2)
print("Sigmoid activation, 25 epochs, batch size 64 model - test set performance:\n Accuracy: %f\n Loss: %f" % (loss_and_metrics[1], loss_and_metrics[0]))
#-----------------------------------------------------------------------------------------
...
#--------------------------Impelment your code here:-------------------------------------
# new_a_model.compile(optimizer=AdamOpt, metrics=['accuracy'], loss='categorical_crossentropy')
new_a_model = load_model("results/init_weights_Sigmoid.h5")
history = new_a_model.fit(BaseX_train, BaseY_train,
                         batch_size=batch_size,
                         epochs=epochs,
                         verbose=1)
...
if not("results" in os.listdir()):
    os.mkdir("results")
save_dir = "results/"
model_name = "final_weights_Sigmoid_40ep.h5"
model_path = os.path.join(save_dir, model_name)
new_a_model.save(model_path)

cur_model = load_model("results/final_weights_Sigmoid_40ep.h5")
loss_and_metrics = cur_model.evaluate(X_test,Y_test, verbose = 2)
print("Sigmoid activation, 40 epochs, batch size 64 model - test set performance:\n Accuracy: %f\n Loss: %f" % (loss_and_metrics[1], loss_and_metrics[0]))
#-----------------------------------------------------------------------------------------
...
if not("results" in os.listdir()):
    os.mkdir("results")
save_dir = "results/"
model_name = "final_weights_RelU_32bs.h5"
model_path = os.path.join(save_dir, model_name)
model_relu.save(model_path)

cur_model = load_model("results/final_weights_RelU_32bs.h5")
loss_and_metrics = cur_model.evaluate(X_test,Y_test, verbose = 2)
print("RelU activation, 50 epochs, batch size 32 model - test set performance:\n Accuracy: %f\n Loss: %f" % (loss_and_metrics[1], loss_and_metrics[0]))
#----------------------------------------------------------------------------------------
...
if not("results" in os.listdir()):
    os.mkdir("results")
save_dir = "results/"
model_name = "final_weights_Sigmoid_batchnorm.h5"
model_path = os.path.join(save_dir, model_name)
new_a_model.save(model_path)

cur_model = load_model("results/final_weights_Sigmoid_batchnorm.h5")
loss_and_metrics = cur_model.evaluate(X_test,Y_test, verbose = 2)
print("Sigmoid activation, 50 epochs, batch size 32, batch normalization model - test set performance:\n Accuracy: %f\n Loss: %f" % (loss_and_metrics[1], loss_and_metrics[0]))
#----------------------------------------------------------------------------------------
...
#--------------------------Impelment your code here:-------------------------------------
def get_net(input_shape,drop,dropRate,reg,num_of_fil):
    #Defining the network architecture:
    model = Sequential()
    model.add(Permute((1,2,3),input_shape = input_shape))
    model.add(Conv2D(filters=num_of_fil[0], kernel_size=(3,3), padding='same', activation='relu',name='Conv2D_1',kernel_regularizer=regularizers.l2(reg)))
    if drop:
        model.add(Dropout(rate=dropRate))
    model.add(BatchNormalization(axis=1))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Conv2D(filters=num_of_fil[1], kernel_size=(3,3), padding='same', activation='relu',name='Conv2D_2',kernel_regularizer=regularizers.l2(reg)))
    if drop:
        model.add(Dropout(rate=dropRate))
    model.add(BatchNormalization(axis=1))
    model.add(Conv2D(filters=num_of_fil[2], kernel_size=(3,3), padding='same', activation='relu',name='Conv2D_3',kernel_regularizer=regularizers.l2(reg)))
    if drop:
        model.add(Dropout(rate=dropRate))
    model.add(BatchNormalization(axis=1))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Conv2D(filters=num_of_fil[3], kernel_size=(3,3), padding='same', activation='relu',name='Conv2D_4',kernel_regularizer=regularizers.l2(reg)))
    if drop:
        model.add(Dropout(rate=dropRate))
    model.add(BatchNormalization(axis=1))
    model.add(Conv2D(filters=num_of_fil[4], kernel_size=(3,3), padding='same', activation='relu',name='Conv2D_5',kernel_regularizer=regularizers.l2(reg)))
    if drop:
        model.add(Dropout(rate=dropRate))
    model.add(BatchNormalization(axis=1))
    model.add(MaxPooling2D(pool_size=(2, 2)))
...
num_of_fil = [32,64,64,128,128]
NNet = get_net(input_shape,drop,dropRate,reg,num_of_fil)
#Compile the network:
NNet.compile(optimizer=AdamOpt, metrics=['acc'], loss='categorical_crossentropy')
h = NNet.fit(x=BaseX_train, y=BaseY_train, batch_size=batch_size, epochs=epochs, verbose=1, validation_split=0, validation_data = (BaseX_val, BaseY_val), shuffle=True)
results = NNet.evaluate(X_test,Y_test)
